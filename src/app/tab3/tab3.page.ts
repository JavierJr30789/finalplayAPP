    ...
    if (userId) {
      this.profileService.getProfile(userId).subscribe(
        ...
        if (profileData) {
          ...
          // Si el perfil no existe, mostrar el formulario
...
saveProfile() {
  if (this.profileForm.valid) {
    const profileData: Profile = {
      ...
      email: this.authService.currentUser?.email || '', // Asegurar que el email esté presente
    };

    this.profileService
      .saveProfile(profileData)
      .then(() => {
        this.showSuccess('Perfil guardado con éxito');
